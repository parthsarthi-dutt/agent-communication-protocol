...
with st.sidebar:
    ...
    intake_provider_label = st.selectbox(
        "Provider", PROVIDER_OPTIONS, index=2, key="intake_prov",
        help="Ollama runs locally (edge), GenAI/Groq run in the cloud."
    )
    intake_provider = PROVIDER_MAP[intake_provider_label]
    intake_model = st.text_input(
        "Model", value=DEFAULT_MODELS[intake_provider], key="intake_model"
    )
    ...
    rec_model = st.text_input(
        "Model", value=DEFAULT_MODELS[rec_provider], key="rec_model"
    )
    ...
    aud_provider_label = st.selectbox(
        "Provider", PROVIDER_OPTIONS, index=1, key="aud_prov",
        help="The auditor cross-checks the recommendation for compliance."
    )
    aud_provider = PROVIDER_MAP[aud_provider_label]
    aud_model = st.text_input(
        "Model", value=DEFAULT_MODELS[aud_provider], key="aud_model"
    )
